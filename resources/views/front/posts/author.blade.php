...
<div class="container">
    <div class="row">
        ...
        <main class="posts-listing col-lg-8"> 
            <div class="container">
                <h2 class="mb-3">Posts by author "{{$author->name}}"</h2>
                <div class="row">
                    @foreach($posts as $post)
                    <!-- post -->
                    <div class="post col-xl-6">
                        <div class="post-thumbnail"><a href="{{ route('front.posts.single',['post'=>$post->id])}}"><img src="/themes/front/img/blog-post-1.jpeg" alt="..." class="img-fluid"></a></div>
                        <div class="post-details">
                            <div class="post-meta d-flex justify-content-between">
                                <div class="date meta-last">{{ $post->giveMeHumanFriendlyDate()}}</div>
                                <div class="category"><a href="{{ route('front.posts.single',['post'=>$post->id])}}">{{ $post->category->name}}</a></div>
                            </div><a href="{{ route('front.posts.single',['post'=>$post->id])}}">
                                <h3 class="h4">{{ $post->title}}</h3></a>
                            <p class="text-muted">{{ $post->shortDescription}}</p>
                            <footer class="post-footer d-flex align-items-center"><a href="{{ route('front.posts.author',['author'=>$author])}}" class="author d-flex align-items-center flex-wrap">
